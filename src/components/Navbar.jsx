...
const Navbar = () => {
  ...
  return (
    <nav className='w-[100vw] h-[4rem] md:h-[5rem] bg-white shadow-sm flex justify-between items-center py-3 md:py-6 px-4 md:px-40'>
      <div className='flex items-center gap-3 cursor-pointer' onClick={()=>{router('/dashboard')}}>
        <StoreIcon />
      ...
      <div>
        ...
        </div> :  <button onClick={()=>{router("/login")}} className={`cursor-pointer text-bold text-lg md:text-xl px-2 py-1 md:px-3 md:py-2 bg-indigo-600 text-white rounded-lg md:rounded-lg`}>Login</button>}
      </div>
    </nav>
  )
}
